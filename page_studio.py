...
def _inicio():
    n = NEGOCIO
    return f'''<section id="inicio" class="hero">
...
def _resultados(fotos):
  ...
  <p><a href="{esc(NEGOCIO['instagram_url'])}" target="_blank" rel="noopener">Ver mais no Instagram @{esc(NEGOCIO['instagram_usuario'])}</a></p>
</section>'''
...
def _combos_pacotes():
    ...
    pacotes = "".join(_cartao(p, "pacote", f"Agendar {p['primeira_sessao'].lower()}",
                              "O pacote é fechado no atendimento.") for p in PACOTES)
  ...
  <p class="secao-intro">Você agenda a primeira sessão e fecha o pacote com a Laryssa no dia.</p>
  <div class="cartoes">{pacotes}</div>
</section>'''
...
def _como_chegar(fotos):
    n = NEGOCIO
    horarios = "".join(f"<li>{esc(dia)}: {abre} às {fecha}</li>" for dia, abre, fecha, _ in HORARIO)
    foto = (f'<figure>{imagem("studio", fotos["studio"], ALT_FOTOS["studio"], sizes="(min-width: 720px) 50vw, 100vw")}</figure>'
            if fotos else "")
    return f'''<section id="como-chegar" class="duas-colunas">
  <div>
    <h2>Como chegar</h2>
